...
const VARIANT_AUDIT_STATE_PATH = String(process.env.VARIANT_AUDIT_STATE_PATH || path.join(path.dirname(AUTO_STATE_PATH), 'primetac-variants-audit.json'));
...
function expectedManagedGroupIds(plan=[]){
  const byId=new Map(MANAGED_GROUPS.map(g=>[String(g.id),g]));
  const out=new Set();
  for(const row of (plan||[])){
    let id=String(row?.group_id||'');
    let guard=0;
    while(id && guard++<20){
      if(out.has(id)) break;
      out.add(id);
      const g=byId.get(id);
      id=g?.parent ? String(g.parent) : '';
    }
  }
  return out;
}
function expectedManagedGroupNames(plan=[]){
  const ids=expectedManagedGroupIds(plan);
  const byId=new Map(MANAGED_GROUPS.map(g=>[String(g.id),g]));
  return new Set([...ids].map(id=>norm(byId.get(id)?.name||'').toLowerCase()).filter(Boolean));
}
...
let autoState = {
  ...
  managed_group_nodes_expected:0,
  managed_group_nodes_expected_found:0,
  ...
  real_groups_mapped:0,
  real_groups_missing:0,
  real_groups_ambiguous:0,
  real_group_map:{},
  real_group_missing_names:[],
  real_group_ambiguous_names:[],
  group_probe:null
};
...
let variantState = {
  ...
  error:null,
  total_positions:0,
  standalone_positions:0,
  existing_variant_positions:0,
  existing_variant_groups:0,
  candidate_families:0,
  high_confidence_families:0,
  medium_confidence_families:0,
  candidate_positions:0,
  high_confidence_positions:0,
  estimated_savings_high:0,
  estimated_cards_high:0,
  deep_candidate_families:0,
  deep_high_confidence_families:0,
  deep_medium_confidence_families:0,
  deep_supplier_backed_families:0,
  deep_supplier_groupid_families:0,
  deep_supplier_groupid_positions:0,
  deep_supplier_groupid_savings:0,
  supplier_records_with_groupid:0,
  deep_sku_backed_families:0,
  estimated_savings_deep:0,
  estimated_cards_deep:0,
  goal_gap_after_deep:0,
  supplier_matches_for_audit:0,
  supplier_audit_error:null,
  target_limit:1000,
  need_to_reduce:0,
  families:[],
  deep_families:[],
...
let variantAuditPromise = null;

function saveVariantAuditSnapshot(){
  try{
    const dir=path.dirname(VARIANT_AUDIT_STATE_PATH);
    fs.mkdirSync(dir,{recursive:true});
    const tmp=VARIANT_AUDIT_STATE_PATH+'.tmp';
    const snap={...variantState,running:false};
    fs.writeFileSync(tmp,JSON.stringify(snap,null,2),'utf8');
    fs.renameSync(tmp,VARIANT_AUDIT_STATE_PATH);
  }catch(_e){}
}
function loadVariantAuditSnapshot(){
  try{
    if(!fs.existsSync(VARIANT_AUDIT_STATE_PATH)) return;
    const saved=JSON.parse(fs.readFileSync(VARIANT_AUDIT_STATE_PATH,'utf8'));
    if(saved && Number(saved.total_positions||0)>0 && Array.isArray(saved.deep_families) && saved.deep_families.length){
      Object.assign(variantState,saved,{running:false,error:null});
    }
  }catch(_e){}
}
loadVariantAuditSnapshot();
...
function supplierRecordFromNode(n, supplier){
  ...
  // Prom/YML varieties are commonly linked by offer attribute group_id. Preserve it from supplier XML.
  const variation_group_id=norm(primitive(
    n['@_group_id'] ?? n['@_groupId'] ?? n.group_id ?? n.groupId ??
    n['@_variation_group_id'] ?? n.variation_group_id ?? n.variant_group_id ??
    n.modification_group_id ?? n.parent_group_id ?? ''
  ));
  ...
  return {supplier,id,sku,name,brand,url,description,pictures,price,category,variation_group_id,available,stock,params,raw_hint:Object.keys(n).slice(0,30)};
}
...
function promStrongSkuCandidates(p){
  const vals=[];
  // These are real product/article fields. Unlike external_id they are much less likely
  // to collide with a supplier's internal offer id from another feed.
  for(const k of ['sku','article','vendor_code','vendorCode','code','presence_sku']){
    if(p && p[k]!==undefined && p[k]!==null) vals.push(primitive(p[k]));
  }
  return uniq(vals.map(cleanSku).filter(Boolean));
}
function promExternalIdCandidates(p){
  const vals=[];
  for(const k of ['external_id','externalId']){
    if(p && p[k]!==undefined && p[k]!==null) vals.push(primitive(p[k]));
  }
  return uniq(vals.map(cleanSku).filter(Boolean));
}
function promSkuCandidates(p){
  return uniq([...promStrongSkuCandidates(p),...promExternalIdCandidates(p)]);
}
function supplierIdentityEvidence(p,r){
  const pn=normalizeNameForMatch(p?.name||''), rn=normalizeNameForMatch(r?.name||'');
  const jac=jaccardName(p?.name||'',r?.name||'');
  const ptype=findType(p?.name||''), rtype=findType(r?.name||'');
  const typeConflict=Boolean(ptype && rtype && ptype!==rtype);
  ...
  const brandSame=Boolean(pb && rb && pb.toLowerCase()===rb.toLowerCase());
  const brandConflict=Boolean(pb && rb && pb.toLowerCase()!==rb.toLowerCase());
  const exactName=Boolean(pn && rn && pn===rn);
  // If both names expose a concrete product type, a jacket can never become a LOWA sneaker
  // just because two unrelated systems happened to use the same numeric id.
  const titleCorroborates=!typeConflict && (exactName || jac>=0.38 || (brandSame && jac>=0.24));
  return {pn,rn,jac,ptype,rtype,typeConflict,pb,rb,brandSame,brandConflict,exactName,titleCorroborates};
}

function supplierScore(p,r){
  const strongSkus=promStrongSkuCandidates(p);
  const externalIds=promExternalIdCandidates(p);
  const rSku=cleanSku(r.sku);
  const rId=cleanSku(r.id);
  const ev=supplierIdentityEvidence(p,r);

  if(!ev.brandConflict && !ev.typeConflict){
    // A genuine SKU/article match is the strongest identity signal.
    if(rSku && strongSkus.includes(rSku)) return {score:100,reason:'точный SKU/артикул'};

    // Supplier offer IDs and Prom external_id are NOT globally unique across feeds.
    // Accept them only when the product title/model independently corroborates the match.
    if(rId && strongSkus.includes(rId) && ev.titleCorroborates) return {score:99,reason:'точный ID поставщика + модель'};
    if(rSku && externalIds.includes(rSku) && ev.titleCorroborates) return {score:98,reason:'external_id = SKU + модель'};
    if(rId && externalIds.includes(rId) && ev.titleCorroborates) return {score:98,reason:'external_id = ID поставщика + модель'};
  }

  if(ev.exactName && !ev.brandConflict && !ev.typeConflict) return {score:96,reason:'точное название'};
  let score=Math.round(ev.jac*90);
  if(ev.brandSame) score+=5;
  if(ev.brandConflict || ev.typeConflict) score=Math.min(score,55);
  const pc=findColor(p?.name||''), rc=findColor(r?.name||'');
  if(pc && rc && pc===rc) score+=3;
  const reason=ev.typeConflict?'сходство названия, но тип товара не совпадает':(ev.brandConflict?'сходство названия, но бренд не совпадает':'сходство названия');
  return {score:Math.min(94,score),reason};
}
...
function buildAutoYml(items){
  ...
  const offers=items.map(({p,match,attrs,managedGroup})=>{
    ...
    // Prom's YML help marks <description> as required. Earlier builds omitted it because
    // updated_fields excluded description. Prom could accept the background job but then
    // process zero offers. Send the current/supplier description only to make the YML
    // structurally valid; updated_fields remains group + attributes, so descriptions are
    // not changed by this import.
    const description=norm(p?.description || match?.record?.description || name);
...
function promImportFile(xmlText,settings){
  ...
  return new Promise((resolve,reject)=>{
    ...
    add(`--${boundary}\r\nContent-Disposition: form-data; name="file"; filename="primetac-groups-attributes.xml"\r\nContent-Type: application/xml\r\n\r\n`);
...
async function verifyLastImportNow(){
  try{
    ...
    const expectedNames=expectedManagedGroupNames(lastAutoImportPlan);
    autoState.managed_group_nodes_expected=expectedNames.size;
    autoState.managed_group_nodes_expected_found=[...expectedNames].filter(n=>maps.idsByName.has(n)).length;
    ...
    if(!lastAutoImportPlan.length){
      ...
      return {
        ...
        managed_group_nodes_expected:autoState.managed_group_nodes_expected,
        managed_group_nodes_expected_found:autoState.managed_group_nodes_expected_found,
        attributes_verified:0,
        attributes_verifiable:0,
...
const AUTO_PARTIAL_NO_EFFECT_MIN=Math.max(10,Number(process.env.AUTO_PARTIAL_NO_EFFECT_MIN||20));
function importIsPartial(status){
  return /PARTIAL/.test(String(status||'').toUpperCase());
}
function statusReportsAllItemsAccounted(counters,totalItems=0){
  const c=counters||{};
  const total=Number.isFinite(Number(c.total)) && Number(c.total)>0 ? Number(c.total) : Number(totalItems||0);
  if(!(total>0)) return false;
  if(Number.isFinite(Number(c.processed)) && Number(c.processed)>=total) return true;
  const keys=['created','updated','not_changed','errors'];
  let seen=false,sum=0;
  for(const k of keys){
    if(Number.isFinite(Number(c[k]))){ seen=true; sum+=Number(c[k]); }
  }
  return seen && sum>=total;
}
function partialNoEffectCanBeReleased(status,counters,totalItems=0,elapsedSec=0){
  return importIsPartial(status) &&
    Number(elapsedSec||0) >= AUTO_PARTIAL_NO_EFFECT_MIN*60 &&
    statusReportsAllItemsAccounted(counters,totalItems) &&
    statusReportsNoEffect(counters);
}
...
function scheduleBackgroundImportWatcher(id,totalItems=0){
  ...
  const tick=async()=>{
    ...
    // Even after the normal background-watch horizon, do one real status/catalog check first.
    // This matters after a Render restart: an old PARTIAL task may already have accounted for
    // every item and can be safely classified instead of being frozen forever by the timeout.
    const watchExpired=elapsed > AUTO_BACKGROUND_WATCH_MAX_MIN*60*1000;
    ...
    if(importSucceeded(terminal) && statusReportsNoEffect(autoState.import_status_counters) && !catalogProvesImport(verify)){
      ...
      autoState.import_error='Prom завершил задачу со статусом SUCCESS, но сообщил 0 созданных и 0 обновлённых товаров, а группы каталога не изменились. Блокировка снята. v3.4.11 создаёт недостающие непустые группы безопасным импортом only_update=false и формирует валидный YML с обязательным description.';
      autoState.phase='⚠️ Prom завершил импорт без изменений. Следующий запуск разрешит создание недостающих групп';
      autoState.progress=100;
      autoState.finished_at=new Date().toISOString();
      if(autoState.restored_after_restart) autoBootGuardSatisfiedByRestoredTerminal=true;
    ...
    // Prom sometimes leaves a fully-accounted import in PARTIAL for a long time.
    // If every offer is already counted, there are zero created/updated/errors, the catalog
    // still shows no group progress, and this state lasted long enough, keeping the lock forever
    // is less safe than classifying this exact task as no-effect.
    if(partialNoEffectCanBeReleased(terminal,autoState.import_status_counters,totalItems,autoState.import_elapsed_sec) && !catalogProvesImport(verify)){
      autoState.import_background=false;
      autoState.import_terminal_confirmed=true;
      autoState.import_status='PARTIAL_NO_EFFECT';
      autoState.import_error=`Prom остаётся в PARTIAL более ${AUTO_PARTIAL_NO_EFFECT_MIN} мин, но все ${totalItems||autoState.group_assignments_planned||0} позиций уже учтены как unchanged и группы не изменились. Старый lock снят; следующий импорт пойдёт с only_update=false.`;
      autoState.phase='⚠️ Старый PARTIAL завершён без эффекта. Можно запускать безопасное создание групп';
      autoState.progress=100;
      autoState.finished_at=new Date().toISOString();
      if(autoState.restored_after_restart) autoBootGuardSatisfiedByRestoredTerminal=true;
      clearImportLock('partial-no-effect');
      return;
    }
    ...
    if(watchExpired){
      if(autoState.import_status==='PREVIOUS_IMPORT_ACTIVE'){
        ...
        autoState.phase=`⚠️ Импорт Prom всё ещё не подтверждён спустя ${AUTO_BACKGROUND_WATCH_MAX_MIN} мин. Последний status/catalog check выполнен; lock сохранён`;
        persistImportLock('background-timeout-after-final-check');
      }
      return;
    }
      ...
      try{
        ...
        if(importSucceeded(status)){
          ...
          if(statusReportsNoEffect(r.counters) && !catalogProvesImport(verified)){
            autoState.import_background=false;
            autoState.import_status='SUCCESS_NO_EFFECT';
            autoState.import_error='Prom завершил задачу SUCCESS, но сообщил 0 созданных и 0 обновлённых товаров; фактические группы также не изменились. Повторный запуск разрешён. v3.4.11 читает все страницы /groups/list, проверяет группы по реальному Prom ID/названию и разрешает создание недостающих групп через only_update=false.';
        ...
        if(partialNoEffectCanBeReleased(status,r.counters,totalItems,autoState.import_elapsed_sec)){
          const verified=await verifyLastImportNow();
          if(!catalogProvesImport(verified)){
            autoState.import_background=false;
            autoState.import_terminal_confirmed=true;
            autoState.import_status='PARTIAL_NO_EFFECT';
            autoState.import_error=`Prom остаётся в PARTIAL более ${AUTO_PARTIAL_NO_EFFECT_MIN} мин, хотя все позиции уже учтены и изменений нет. Повторный запуск разрешён с only_update=false для создания недостающих групп.`;
            autoState.phase='⚠️ PARTIAL без эффекта · старый импорт разблокирован';
            clearImportLock('partial-no-effect');
            return {ok:false,status:'PARTIAL_NO_EFFECT',data:last,counters:r.counters,pending:false,verified};
          }
        }
...
function promGroupDisplayName(input){
  if(!input || typeof input!=='object') return '';
  const ml=input.name_multilang||input.nameMultilang||{};
  return norm(input.name||input.title||ml.uk||ml.ua||ml.ru||input.translation?.name||'');
}
function flattenPromGroups(input,out=[],parentHint=''){
  if(!input) return out;
  if(Array.isArray(input)){
    for(const x of input) flattenPromGroups(x,out,parentHint);
    return out;
  }
  if(typeof input!=='object') return out;

  const ownId=input.id!==undefined ? String(input.id) : '';
  const ownName=promGroupDisplayName(input);
  const ownParent=String(input.parent_id||input.parentId||input.parent?.id||parentHint||'');
  if(ownId && ownName){
    out.push({id:ownId,name:ownName,parent_id:ownParent});
  }

  const childParent=ownId||parentHint||'';
  for(const key of ['groups','children','subgroups','items','nodes']){
    if(input[key]) flattenPromGroups(input[key],out,childParent);
  }
  return out;
}
async function getPromGroups(){
  // /groups/list is paginated. v3.4.7 read only the first 100 groups, which can
  // make an existing PrimeTac tree look like 1/46 simply because most nodes are
  // on later pages. Read every page before verifying or auditing the structure.
  const all=[];
  const seen=new Set();
  let lastId=null;
  let pages=0;
  ...
  while(guard++<100){
    const url=`/groups/list?limit=100${lastId ? `&last_id=${encodeURIComponent(lastId)}` : ''}`;
    const res=await promRequest('GET',url);
    const data=res.data||{};
    pages++;

    const topBatch=Array.isArray(data?.groups)
      ? data.groups
      : (Array.isArray(data?.items) ? data.items : (Array.isArray(data) ? data : []));
    const flat=flattenPromGroups(topBatch.length ? topBatch : data,[]);
    ...
    const explicitNext = data?.last_id ?? data?.lastId ?? data?.next_last_id ?? data?.pagination?.last_id ?? data?.pagination?.lastId;
    const fallbackNext = topBatch.length>=100 ? topBatch[topBatch.length-1]?.id : null;
    const next = explicitNext ?? fallbackNext;

    if(!next || String(next)===String(lastId) || !added) break;
    lastId=String(next);

    if(topBatch.length && topBatch.length<100 && explicitNext==null) break;
  }

  autoState.groups_pages_audited=pages;
  autoState.groups_audited=all.length;
  return all;
}
...
function buildRealPromGroupMapping(groups=[]){
  ...
  const byManagedId=new Map(MANAGED_GROUPS.map(g=>[String(g.id),g]));
  const depthOf=(g)=>{let d=0,id=String(g?.parent||''),guard=0; while(id&&guard++<20){d++;id=String(byManagedId.get(id)?.parent||'');} return d;};
  const ordered=[...MANAGED_GROUPS].sort((a,b)=>depthOf(a)-depthOf(b));
  const mapping={}; const missing=[]; const ambiguous=[];
  for(const mg of ordered){
    const key=groupNameKey(mg.name);
    let candidates=[...(byName.get(key)||[])];
    const parentMapped=mg.parent ? mapping[String(mg.parent)] : null;
    if(parentMapped && candidates.length>1){
      const narrowed=candidates.filter(x=>String(x.parent_id||'')===String(parentMapped.prom_id));
      if(narrowed.length) candidates=narrowed;
    }
    if(candidates.length===1){
      const g=candidates[0];
      mapping[String(mg.id)]={managed_id:String(mg.id),managed_name:mg.name,prom_id:String(g.id),prom_name:g.name,parent_prom_id:String(g.parent_id||''),status:'mapped'};
    }else if(candidates.length===0){
      missing.push(mg.name);
    }else{
      ambiguous.push(mg.name);
      mapping[String(mg.id)]={managed_id:String(mg.id),managed_name:mg.name,prom_id:'',prom_name:'',status:'ambiguous',candidates:candidates.map(x=>({id:String(x.id),name:x.name,parent_id:String(x.parent_id||'')}))};
    }
  }
  return {mapping,missing,ambiguous};
}
async function refreshRealPromGroupMapping(){
  const groups=await getPromGroups();
  const r=buildRealPromGroupMapping(groups);
  autoState.real_group_map=r.mapping;
  autoState.real_groups_mapped=Object.values(r.mapping).filter(x=>x.status==='mapped'&&x.prom_id).length;
  autoState.real_groups_missing=r.missing.length;
  autoState.real_groups_ambiguous=r.ambiguous.length;
  autoState.real_group_missing_names=r.missing;
  autoState.real_group_ambiguous_names=r.ambiguous;
  return {...r,groups};
}
function mappedPromGroupId(managedGroup){
  return String(autoState.real_group_map?.[String(managedGroup?.id||'')]?.prom_id||'');
}
async function runRealGroupProbe(){
  if(autoState.group_probe?.running) return autoState.group_probe;
  const probe={running:true,started_at:new Date().toISOString(),ok:false,status:'START',error:null};
  autoState.group_probe=probe;
  try{
    const map=await refreshRealPromGroupMapping();
    const products=await listAllProducts();
    let candidate=null;
    for(const p of products){
      const mg=classifyManagedGroup(p,null);
      if(!mg || mg.fallback) continue;
      const target=mappedPromGroupId(mg);
      if(!target) continue;
      const before=String(autoGroupId(p)||'');
      if(before && before===target) continue;
      candidate={p,mg,target,before};
      break;
    }
    if(!candidate) throw new Error('Не найден товар для безопасного теста: либо все уже в нужных группах, либо не удалось сопоставить группы Prom.');
    probe.product_id=String(candidate.p.id);
    probe.product_name=norm(candidate.p.name||'');
    probe.managed_group=candidate.mg.name;
    probe.target_prom_group_id=candidate.target;
    probe.before_group_id=candidate.before;
    probe.status='WRITE_ONE';
    const gid=/^\d+$/.test(candidate.target)?Number(candidate.target):candidate.target;
    probe.api_response=(await promRequest('POST','/products/edit',[{id:Number(candidate.p.id),group_id:gid}])).data;
    await new Promise(r=>setTimeout(r,1800));
    const fresh=await getProduct(candidate.p.id);
    probe.after_group_id=String(autoGroupId(fresh)||'');
    probe.ok=probe.after_group_id===String(candidate.target);
    probe.status=probe.ok?'VERIFIED':'NO_EFFECT';
    if(!probe.ok) probe.error='Prom принял products/edit, но group_id товара не изменился. Массовое распределение заблокировано.';
  }catch(e){
    probe.ok=false; probe.status='ERROR'; probe.error=safeText(e);
  }finally{
    probe.running=false; probe.finished_at=new Date().toISOString();
  }
  return probe;
}
...
async function autoImportCharacteristics(){
  ...
  const expectedNamesForPlan=expectedManagedGroupNames(lastAutoImportPlan);
  autoState.managed_group_nodes_expected=expectedNamesForPlan.size;

  // v3.4.11: groups already exist in Prom. Do NOT try to recreate them through YML.
  // First map our fixed names to the real Prom IDs and require a verified one-product API probe.
  await refreshRealPromGroupMapping();
  if(autoState.real_groups_missing || autoState.real_groups_ambiguous){
    autoState.import_status='WAIT_REAL_GROUP_MAPPING';
    autoState.import_error=`Сопоставлено реальных групп Prom ${autoState.real_groups_mapped}/${MANAGED_GROUPS.length}; отсутствует ${autoState.real_groups_missing}; неоднозначно ${autoState.real_groups_ambiguous}. YML-импорт групп отключён.`;
    autoState.phase='4/5 Реальные группы Prom: требуется проверить сопоставление';
    autoState.progress=82;
    return {pending:false,status:'WAIT_REAL_GROUP_MAPPING',mapping_required:true};
  }
  if(!autoState.group_probe?.ok){
    autoState.import_status='WAIT_GROUP_PROBE';
    autoState.import_error='Все группы сопоставлены с реальными ID Prom. Перед массовым переносом нужен безопасный тест одного товара через products/edit → group_id.';
    autoState.phase='4/5 Группы Prom сопоставлены. Нужен тест 1 товара';
    autoState.progress=84;
    return {pending:false,status:'WAIT_GROUP_PROBE',probe_required:true};
  }

  // Probe passed: stop here on purpose. v3.4.11 is a diagnostic/safety bridge and must
  // not fall through to the old YML group-import path. Mass direct group writes come next.
  autoState.import_status='GROUP_PROBE_VERIFIED';
  autoState.import_error='Тест одного товара подтверждён. YML-импорт групп отключён. Массовый перенос в v3.4.11 намеренно не запускается.';
  autoState.phase='✅ Тест реального group_id подтверждён. Готово к массовому переносу в следующей версии';
  autoState.progress=100;
  return {pending:false,status:'GROUP_PROBE_VERIFIED',probe_verified:true};
  ...
  // Safety for only_update=false: every offer must still be an already-known Prom product.
  // We build `items` from /products/list, but additionally refuse the import if external IDs
  // are duplicated. That prevents an ambiguous import from accidentally creating a second item.
  const extIds=items.map(x=>autoExternalId(x.p));
  const uniqueExtIds=new Set(extIds);
  if(uniqueExtIds.size!==items.length){
    const duplicates=extIds.filter((id,i,a)=>id && a.indexOf(id)!==i).slice(0,10);
    throw new Error('Безопасность импорта: в текущем каталоге Prom есть повторяющиеся external_id: '+duplicates.join(', '));
  }
  ...
  const settings={
    ...
    // IMPORTANT: false is required here so Prom may create missing non-empty groups from <categories>.
    // Safety: `items` is built ONLY from products just read from this same Prom catalog, and
    // products without external_id are skipped. We still update only group + attributes.
    only_update:false,
  ...
  if(!st.ok){
    if(st.status==='SUCCESS_NO_EFFECT' || st.status==='PARTIAL_NO_EFFECT'){
      autoState.import_status=st.status;
...
async function runAutoAll(reason='manual'){
  ...
  Object.assign(autoState,{
    ...
    restored_after_restart:false,
    restored_lock_age_sec:0,
    real_groups_mapped:autoState.real_groups_mapped||0,
    real_groups_missing:autoState.real_groups_missing||0,
    real_groups_ambiguous:autoState.real_groups_ambiguous||0,
    real_group_map:autoState.real_group_map||{},
    real_group_missing_names:autoState.real_group_missing_names||[],
    real_group_ambiguous_names:autoState.real_group_ambiguous_names||[],
    group_probe:autoState.group_probe||null
  });
  ...
  try{
    ...
    if(['SUCCESS_NO_EFFECT','PARTIAL_NO_EFFECT','WAIT_REAL_GROUP_MAPPING','WAIT_GROUP_PROBE','GROUP_PROBE_VERIFIED'].includes(importResult?.status)){
      autoState.current=0;
      autoState.current_total=0;
      return;
    }
...
function variationGroupIdOf(p){
  return norm(
    p?.variation_group_id ?? p?.variationGroupId ?? p?.variant_group_id ?? p?.variantGroupId ??
    p?.modification_group_id ?? p?.modificationGroupId ?? ''
  );
}
function variationBaseIdOf(p){
  return norm(p?.variation_base_id ?? p?.variationBaseId ?? p?.variant_base_id ?? p?.variantBaseId ?? '');
}
function isExistingVariation(p){
  const raw=p?.is_variation ?? p?.isVariation ?? p?.variation ?? false;
  return Boolean(raw===true || raw===1 || raw==='1' || String(raw).toLowerCase()==='true' || variationGroupIdOf(p) || variationBaseIdOf(p));
}
function variantNumericSizes(text){
  const src=String(text||'').toUpperCase();
  const out=[];
  for(const m of src.matchAll(/(?:^|[^0-9])(2[8-9]|3[0-9]|4[0-9]|5[0-9]|6[0-2])(?:[^0-9]|$)/g)) out.push(m[1]);
  return uniq(out);
}
function variantSizesOf(p){
  const fromAttrs=getActualSizes(p)||[];
  const direct=[];
  for(const k of ['size','sizes','product_size','clothes_size','shoe_size']){
    const v=p?.[k];
    if(Array.isArray(v)) direct.push(...v.map(norm));
    else if(v!==undefined && v!==null) direct.push(...detectSizes(v),...variantNumericSizes(v));
  }
  return uniq([
    ...fromAttrs,
    ...direct,
    ...detectSizes(p?.name||''),
    ...variantNumericSizes(p?.name||'')
  ]).filter(Boolean);
}
function variantColorOf(p){
  return norm(getActualColor(p) || getColor(p) || findColor(p?.name||'') || actualAttrValue(p,'color') || '');
}
function variantBrandOf(p){
  return norm(getActualProducer(p) || findBrand(p?.name||'') || '');
}
function variantGroupKeyOf(p){
  const m=classifyManagedGroup(p,supplierMatches.get(String(p?.id))||null);
  return norm(m?.name || autoGroupId(p) || getCategory(p) || '');
}
function variantBaseName(raw){
  let x=norm(raw||'').toLowerCase();
  if(!x) return '';
  // Remove color words. Keep model names and product type so unrelated products do not collapse together.
  x=x.replace(/\b(чорн(?:ий|а|е|і)?|черн(?:ый|ая|ое|ые)?|black|сір(?:ий|а|е|і)?|сер(?:ый|ая|ое|ые)?|grey|gray|хакі|хаки|khaki|койот|coyote|tan|олив(?:а|ковий|ковый)?|olive|зел(?:ений|еный|ёный|ена|ёная)?|green|помаранчев(?:ий|а)?|оранжев(?:ый|ая)?|orange|білий|белый|white|син(?:ій|ий|яя|ій)?|blue|navy|multicam|мультикам|піксель|пиксель|foliage)\b/giu,' ');
  x=x.replace(/\b(?:розмір|размер|size)\s*[:№#-]?\s*(?:xs|s|m|l|xl|[2-6]xl|2[8-9]|3[0-9]|4[0-9]|5[0-9]|6[0-2])\b/giu,' ');
  x=x.replace(/\b(?:xs|[2-6]xl)\b/giu,' ');
  // Single-letter sizes need stronger boundaries so model letters are not destroyed.
  x=x.replace(/(?:^|[\s,;()\[\]/-])(s|m|l|xl)(?=$|[\s,;()\[\]/-])/giu,' ');
  x=x.replace(/(?:^|[^0-9])(2[8-9]|3[0-9]|4[0-9]|5[0-9]|6[0-2])(?:[^0-9]|$)/g,' ');
  x=x.replace(/\b(розмір|размер|size)\b/giu,' ');
  x=x.replace(/[^a-zа-яіїєґ0-9]+/giu,' ').replace(/\s+/g,' ').trim();
  return x;
}
function variantSignature(p){
  const sizes=variantSizesOf(p);
  const color=variantColorOf(p);
  const parts=[];
  if(color) parts.push('c:'+color.toLowerCase());
  if(sizes.length) parts.push('s:'+sizes.slice().sort().join('/').toLowerCase());
  return parts.join('|');
}
function variantPrice(p){
  const n=Number(String(p?.price??'').replace(',','.').replace(/[^0-9.]/g,''));
  return Number.isFinite(n)&&n>0?n:null;
}
function familyPriceSpread(items){
  const ps=items.map(variantPrice).filter(x=>x!==null);
...
function strongVariantSupplierIdentity(p,m){
  ...
  if(!(reason.includes('точный sku') || reason.includes('точный id') || reason.includes('external_id') || reason.includes('точное название'))) return false;
  const ev=supplierIdentityEvidence(p,m.record);
  if(ev.brandConflict || ev.typeConflict) return false;
  // Exact SKU/article is allowed without title corroboration. Numeric supplier IDs / external_id
  // are accepted only when the model/title independently agrees. This blocks cross-feed id collisions.
  if(reason.includes('точный sku/артикул')) return true;
  if(reason.includes('точное название')) return ev.exactName;
  return ev.titleCorroborates;
}
function variantSupplierVariationGroupOf(p){
  const m=variantSupplierMatchOf(p);
  return norm(m?.record?.variation_group_id||'');
}
function variantSupplierSizes(match){
  const r=match?.record||{};
  const out=[];
  for(const [k,v] of Object.entries(r.params||{})){
    const nk=normalizeLabel(k);
    if(/розм|размер|size|shoe|обув|взут|eu\b/.test(nk)){
      out.push(...detectSizes(v),...variantNumericSizes(v));
    }
  }
  out.push(...detectSizes(r.name||''),...variantNumericSizes(r.name||''));
  return uniq(out.map(norm).filter(Boolean));
}
function variantSupplierColor(match){
  const r=match?.record||{};
  for(const [k,v] of Object.entries(r.params||{})){
    const nk=normalizeLabel(k);
    if(/колір|цвет|color/.test(nk)){
      const val=norm(v); if(val) return val;
    }
  }
  return norm(findColor(r.name||'')||'');
}
function variantDeepSizes(p){
  return uniq([...(variantSizesOf(p)||[]),...variantSupplierSizes(variantSupplierMatchOf(p))]).filter(Boolean);
}
function variantDeepColor(p){
  return norm(variantColorOf(p)||variantSupplierColor(variantSupplierMatchOf(p))||'');
}
function variantDeepSignature(p){
  const sizes=variantDeepSizes(p);
  const color=variantDeepColor(p);
  const parts=[];
  if(color) parts.push('c:'+color.toLowerCase());
  if(sizes.length) parts.push('s:'+sizes.slice().sort().join('/').toLowerCase());
  return parts.join('|');
}
function variantSkuStemValue(raw){
  let x=norm(raw||'').toLowerCase();
  if(!x) return '';
  x=x.replace(/[()\[\]]/g,'-').replace(/[\s_.\/]+/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'');
  if(!x) return '';
  const colors=new Set(['black','white','grey','gray','green','olive','khaki','coyote','tan','navy','blue','orange','multicam','foliage','чорний','чорна','черный','черная','сірий','серый','хаки','хакі','койот','олива','оливковий','оливковый','зелений','зеленый','білий','белый','синій','синий','мультикам','пиксель','піксель']);
  let parts=x.split('-').filter(Boolean);
  while(parts.length>1){
    const t=parts[parts.length-1];
    if(colors.has(t) || /^(xs|s|m|l|xl|[2-6]xl)$/.test(t) || /^(2[8-9]|3[0-9]|4[0-9]|5[0-9]|6[0-2])$/.test(t)) parts.pop();
    else break;
  }
  const stem=parts.join('-').replace(/-+/g,'-');
  return stem.length>=4?stem:'';
}
function variantSkuStemOf(p){
  const m=variantSupplierMatchOf(p);
  const vals=[m?.record?.sku,p?.sku,p?.vendor_code,p?.vendorCode,p?.article,p?.external_id,p?.externalId]
    .map(variantSkuStemValue).filter(Boolean);
  if(!vals.length) return '';
  vals.sort((a,b)=>b.length-a.length);
  return vals[0];
}
function variantSupplierBaseOf(p){
  const m=variantSupplierMatchOf(p);
  if(!m || Number(m.score||0)<70) return '';
  return variantBaseName(m.record?.name||'');
}
function variantBrandDeepOf(p){
  const m=variantSupplierMatchOf(p);
  return norm(variantBrandOf(p)||m?.record?.brand||findBrand(m?.record?.name||'')||'');
}
function variantDistinctiveTokens(text){
  const stop=new Set(['тактичний','тактична','тактичне','тактический','тактическая','тактическое','чоловічий','мужской','жіночий','женский','для','і','и','та','the','with','без','з','с','на','одяг','одежда','взуття','обувь','куртка','кофта','фліс','флис','штани','брюки','футболка','сорочка','рубашка','черевики','ботинки','кросівки','кроссовки']);
  return variantBaseName(text).split(' ').filter(t=>t.length>=3&&!stop.has(t));
}
function variantNameSimilarity(a,b){
  const A=new Set(variantDistinctiveTokens(a)), B=new Set(variantDistinctiveTokens(b));
  if(!A.size||!B.size) return 0;
  let inter=0; for(const x of A) if(B.has(x)) inter++;
  const union=new Set([...A,...B]).size;
  const jac=union?inter/union:0;
  const contain=inter/Math.min(A.size,B.size);
  return Math.max(jac, contain*0.92);
}
function buildVariantFamilyFromItems(items, meta={}){
  if(!items || items.length<2) return null;
  const authoritative=Boolean(meta.authoritative);
  const sigs=items.map(variantDeepSignature);
  const informative=sigs.filter(Boolean);
  const uniqueSigs=new Set(informative);
  // Exact supplier group_id is itself an authoritative old-variation signal.
  // For other discovery methods require at least two different size/color signatures.
  if(uniqueSigs.size<2 && !authoritative) return null;
  const sizes=uniq(items.flatMap(variantDeepSizes));
  const colors=uniq(items.map(variantDeepColor).filter(Boolean));
  const names=uniq(items.map(x=>norm(x?.name||'')).filter(Boolean));
  const spread=familyPriceSpread(items);
  const allInformative=informative.length===items.length;
  const mostlyInformative=informative.length>=Math.ceil(items.length*0.70);
  let confidence=meta.confidence||'medium';
  if(meta.source==='strict' && allInformative && spread<=0.35) confidence='high';
  if(meta.source==='supplier_group_id'){
    confidence=(spread<=1.20 ? 'high' : 'medium');
  }
  if(meta.source==='supplier' && allInformative && Number(meta.min_score||0)>=88 && spread<=0.45) confidence='high';
  if(meta.source==='sku' && allInformative && items.length>=2 && spread<=0.45) confidence='high';
  if(!authoritative && (!mostlyInformative || spread>0.70)) confidence='review';
  if(authoritative && spread>2.0) confidence='review';
  return {
    key:meta.key||'', base:meta.base||variantBaseName(items[0]?.name||''), group:meta.group||variantGroupKeyOf(items[0]),
    brand:meta.brand||variantBrandDeepOf(items[0]), count:items.length, savings:items.length-1, confidence,
    source:meta.source||'strict', source_label:meta.source_label||meta.source||'strict', supplier:meta.supplier||'',
    supplier_variation_group_id:meta.variation_group_id||'', authoritative,
    min_supplier_score:meta.min_score||0, sizes, colors, price_spread_pct:Math.round(spread*100), unique_signatures:uniqueSigs.size,
    names:names.slice(0,24), ids:items.map(x=>String(x?.id||'')).filter(Boolean),
    skus:items.map(x=>norm(x?.sku||x?.external_id||'')).filter(Boolean).slice(0,24)
  };
}
function dedupeVariantFamilies(proposals){
  const rank={high:3,medium:2,review:1};
  const sourceRank={supplier_group_id:5,strict:4,supplier:3,sku:2,fuzzy:1};
  const byIds=new Map();
  for(const f of proposals.filter(Boolean)){
    const idKey=[...f.ids].sort().join(',');
    const prev=byIds.get(idKey);
    if(!prev || (rank[f.confidence]||0)>(rank[prev.confidence]||0) || ((rank[f.confidence]||0)===(rank[prev.confidence]||0) && (sourceRank[f.source]||0)>(sourceRank[prev.source]||0))) byIds.set(idKey,f);
  }
  const arr=[...byIds.values()].sort((a,b)=>(rank[b.confidence]-rank[a.confidence]) || b.savings-a.savings || (sourceRank[b.source]-sourceRank[a.source]));
  const claimed=new Set(), selected=[];
  for(const f of arr){
    const free=f.ids.filter(id=>!claimed.has(id));
    if(free.length<2) continue;
    if(free.length!==f.ids.length){
      // Avoid partial, potentially inconsistent families in the estimate.
      continue;
    }
    selected.push(f); f.ids.forEach(id=>claimed.add(id));
  }
  return selected;
}
function auditVariantFamilies(products){
  const standalone=[];
  const existing=[];
  const existingGroups=new Set();
  for(const p of products){
    if(isExistingVariation(p)){
      existing.push(p);
      const g=variationGroupIdOf(p)||variationBaseIdOf(p);
  ...
  const proposals=[];
  let skippedWeak=0;
  const addBuckets=(map,source)=>{
    for(const [key,b] of map){
      if(b.items.length<2) continue;
      const fam=buildVariantFamilyFromItems(b.items,{key,base:b.base,group:b.group,brand:b.brand,source,source_label:b.label||source,min_score:b.min_score||0,supplier:b.supplier||''});
      if(fam && fam.confidence!=='review') proposals.push(fam);
    }
  };

  // Pass 1: strict title/base-name grouping (v3.4.12 logic, but enriched dimensions).
  const strict=new Map();
  for(const p of standalone){
    const base=variantBaseName(p?.name||'');
    if(!base || base.length<7 || base.split(' ').length<2){ skippedWeak++; continue; }
    const group=variantGroupKeyOf(p), brand=variantBrandDeepOf(p);
    const key=[group.toLowerCase(),brand.toLowerCase(),base].join('||');
    if(!strict.has(key)) strict.set(key,{key,base,group,brand,items:[]});
    strict.get(key).items.push(p);
  }
  addBuckets(strict,'strict');

  // Pass 2: exact supplier YML group_id. This is the original variation-family key when the supplier feed contains it.
  const supplierGroupId=new Map();
  for(const p of standalone){
    const m=variantSupplierMatchOf(p);
    ...
    const b=supplierGroupId.get(key); b.items.push(p); b.min_score=Math.min(b.min_score,Number(m.score||0));
  }
  for(const [key,b] of supplierGroupId){
    if(b.items.length<2) continue;
    const fam=buildVariantFamilyFromItems(b.items,{
      key, base:variantSupplierBaseOf(b.items[0])||variantBaseName(b.items[0]?.name||'')||('XML group '+b.gid),
      group:variantGroupKeyOf(b.items[0]), brand:variantBrandDeepOf(b.items[0]),
      source:'supplier_group_id', source_label:'точный group_id из XML поставщика', supplier:b.supplier,
      min_score:b.min_score, variation_group_id:b.gid, authoritative:true, confidence:'high'
    });
    if(fam && fam.confidence!=='review') proposals.push(fam);
  }

  // Pass 3: canonical supplier names. Supplier feeds often keep model names cleaner than Prom titles.
  const sup=new Map();
  for(const p of standalone){
    const m=variantSupplierMatchOf(p); if(!m || Number(m.score||0)<70) continue;
    const base=variantSupplierBaseOf(p); if(!base || base.length<5) continue;
    const group=variantGroupKeyOf(p), brand=variantBrandDeepOf(p), supplier=m.supplier||m.supplier_name||'';
    const key=['supplier',supplier,String(m.record?.category||''),group.toLowerCase(),brand.toLowerCase(),base].join('||');
    if(!sup.has(key)) sup.set(key,{key,base,group,brand,supplier,items:[],min_score:999,label:'поставщик '+(m.supplier_name||supplier)});
    const b=sup.get(key); b.items.push(p); b.min_score=Math.min(b.min_score,Number(m.score||0));
  }
  addBuckets(sup,'supplier');

  // Pass 4: SKU stem. Useful when title wording changed but variants share an article prefix.
  const sku=new Map();
  for(const p of standalone){
    const stem=variantSkuStemOf(p); if(!stem) continue;
    const group=variantGroupKeyOf(p), brand=variantBrandDeepOf(p);
    const key=['sku',group.toLowerCase(),brand.toLowerCase(),stem].join('||');
    if(!sku.has(key)) sku.set(key,{key,base:variantSupplierBaseOf(p)||variantBaseName(p?.name||'')||stem,group,brand,items:[],label:'общий SKU/артикул '+stem});
    sku.get(key).items.push(p);
  }
  addBuckets(sku,'sku');

  let selected=dedupeVariantFamilies(proposals);
  const selectedIds=new Set(selected.flatMap(f=>f.ids));

  // Pass 5: conservative fuzzy review among leftovers. Not counted as safe savings.
  const left=standalone.filter(p=>!selectedIds.has(String(p?.id||'')));
  const byContext=new Map();
  for(const p of left){
    const group=variantGroupKeyOf(p).toLowerCase(), brand=variantBrandDeepOf(p).toLowerCase();
    const k=group+'||'+brand;
    if(!byContext.has(k)) byContext.set(k,[]); byContext.get(k).push(p);
  }
  const fuzzy=[];
  for(const items of byContext.values()){
    if(items.length>180) continue;
    const used=new Set();
    for(let i=0;i<items.length;i++){
      ...
      for(let j=i+1;j<items.length;j++){
        ...
        const sim=variantNameSimilarity(items[i]?.name||'',items[j]?.name||'');
        if(sim<0.84) continue;
        const s1=variantDeepSignature(items[i]), s2=variantDeepSignature(items[j]);
        if(!s1||!s2||s1===s2) continue;
        fam.push(items[j]); used.add(j);
      }
      if(fam.length>=2){
        const f=buildVariantFamilyFromItems(fam,{source:'fuzzy',source_label:'похожие названия — только ручная проверка',confidence:'review'});
        if(f) fuzzy.push(f);
      }
    }
  }
  selected=[...selected,...fuzzy].sort((a,b)=>({high:3,medium:2,review:1}[b.confidence]-({high:3,medium:2,review:1}[a.confidence])) || b.savings-a.savings || a.base.localeCompare(b.base,'ru'));
  return {standalone,existing,existingGroups,families:selected,skippedWeak};
}

async function robustSyncSuppliersForVariants(products, opts={}){
  const tries=Math.max(2,Number(opts.tries||4));
  const minMatches=Math.max(2,Math.floor((products?.length||0)*0.25));
  let lastErr=null;
  promRawCache=new Map((products||[]).map(p=>[String(p.id),p]));

  for(let attempt=0;attempt<tries;attempt++){
    try{
      // If another supplier refresh is running, wait for it instead of treating the catalogue as empty.
      let waitGuard=0;
      while(supplierState.loading && waitGuard<120){ await new Promise(r=>setTimeout(r,250)); waitGuard++; }
      await refreshSupplierFeeds();

      const bez=(supplierRecords.bezet||[]).length;
      const mil=(supplierRecords.militaris||[]).length;
      const gids=Object.values(supplierRecords||{}).flat().filter(r=>numericVariationGroupId(r?.variation_group_id||'')).length;
      if(!bez || !mil || !gids) throw new Error(`фиды ещё не готовы: BEZET=${bez}, Militaris=${mil}, записей group_id=${gids}`);

      await matchSupplierCatalog();
      if(supplierMatches.size<minMatches) throw new Error(`слишком мало сопоставлений Prom↔XML: ${supplierMatches.size}/${products.length}`);
      return {ok:true,bezet:bez,militaris:mil,groupid_records:gids,matches:supplierMatches.size,attempt:attempt+1};
    }catch(e){
      lastErr=e;
  ...
  // Last-chance recovery: if records were loaded on any attempt, rematch them even if the latest HTTP refresh failed.
  try{
    const bez=(supplierRecords.bezet||[]).length;
    const mil=(supplierRecords.militaris||[]).length;
    const gids=Object.values(supplierRecords||{}).flat().filter(r=>numericVariationGroupId(r?.variation_group_id||'')).length;
    if(bez && mil && gids){
      await matchSupplierCatalog();
      if(supplierMatches.size>=2) return {ok:true,stale:true,bezet:bez,militaris:mil,groupid_records:gids,matches:supplierMatches.size};
    }
  }catch(e){ lastErr=e; }
  throw new Error('Не удалось восстановить данные поставщиков для разновидностей: '+safeText(lastErr));
}

async function runVariantAudit(opts={}){
  if(variantAuditPromise) return await variantAuditPromise;
  const forceSuppliers=Boolean(opts && opts.forceSuppliers);
  variantAuditPromise=(async()=>{
    variantState.running=true;
    variantState.started_at=new Date().toISOString();
    variantState.finished_at=null;
    variantState.error=null;
    let supplierAuditError=null;
    try{
      let products=[];
      let lastProductsError=null;
      for(let attempt=0;attempt<3;attempt++){
        try{
          products=await listAllProducts();
          if(products.length) break;
          lastProductsError=new Error('Prom API вернул пустой список товаров');
        }catch(e){ lastProductsError=e; }
        if(attempt<2) await new Promise(r=>setTimeout(r,1200*(attempt+1)));
      }
      if(!products.length){
        throw new Error('Аудит не получил товары Prom после 3 попыток. Старый успешный отчёт сохранён. Последняя ошибка: '+safeText(lastProductsError));
      }
      // Keep the product snapshot in RAM so supplier matching and probe use the same catalogue.
      promRawCache=new Map(products.map(p=>[String(p.id),p]));
      try{
        if(forceSuppliers || !supplierMatches.size || supplierMatches.size<Math.floor(products.length*0.50)) {
          await robustSyncSuppliersForVariants(products,{tries:4});
        }
      }catch(e){
        supplierAuditError=safeText(e);
        // A supplier outage must not replace a previously successful XML group_id audit with a zero-family snapshot.
        if(Number(variantState.deep_supplier_groupid_families||0)>0 && Array.isArray(variantState.deep_families) && variantState.deep_families.length){
          throw new Error('Фиды временно недоступны. Сохраняю предыдущий успешный аудит: '+supplierAuditError);
        }
      }
      const r=auditVariantFamilies(products);
      const strictHigh=r.families.filter(x=>x.confidence==='high' && x.source==='strict');
      const allHigh=r.families.filter(x=>x.confidence==='high');
      const allMedium=r.families.filter(x=>x.confidence==='medium');
      const counted=r.families.filter(x=>x.confidence==='high' || x.confidence==='medium');
      const strictSavings=strictHigh.reduce((n,x)=>n+x.savings,0);
      const deepSavings=counted.reduce((n,x)=>n+x.savings,0);
      Object.assign(variantState,{
        supplier_audit_error:supplierAuditError,
        total_positions:products.length,
        standalone_positions:r.standalone.length,
        existing_variant_positions:r.existing.length,
        existing_variant_groups:r.existingGroups.size,
        candidate_families:r.families.length,
        high_confidence_families:allHigh.length,
        medium_confidence_families:allMedium.length,
        candidate_positions:r.families.reduce((n,x)=>n+x.count,0),
        high_confidence_positions:allHigh.reduce((n,x)=>n+x.count,0),
        estimated_savings_high:strictSavings,
        estimated_cards_high:Math.max(0,products.length-strictSavings),
        deep_candidate_families:counted.length,
        deep_high_confidence_families:allHigh.length,
        deep_medium_confidence_families:allMedium.length,
        deep_supplier_backed_families:counted.filter(x=>x.source==='supplier').length,
        deep_supplier_groupid_families:counted.filter(x=>x.source==='supplier_group_id').length,
        deep_supplier_groupid_positions:counted.filter(x=>x.source==='supplier_group_id').reduce((n,x)=>n+x.count,0),
        deep_supplier_groupid_savings:counted.filter(x=>x.source==='supplier_group_id').reduce((n,x)=>n+x.savings,0),
        supplier_records_with_groupid:Object.values(supplierRecords||{}).flat().filter(r=>norm(r?.variation_group_id||'')).length,
        deep_sku_backed_families:counted.filter(x=>x.source==='sku').length,
        estimated_savings_deep:deepSavings,
        estimated_cards_deep:Math.max(0,products.length-deepSavings),
        goal_gap_after_deep:Math.max(0,Math.max(0,products.length-deepSavings)-variantState.target_limit),
        supplier_matches_for_audit:supplierMatches.size,
        need_to_reduce:Math.max(0,products.length-variantState.target_limit),
        families:r.families.filter(x=>x.source==='strict'),
        deep_families:r.families,
        skipped_weak:r.skippedWeak,
        notes:[
          'Аудит только читает каталог Prom и фиды поставщиков; ничего в Prom не изменяет.',
          'Если аудит уже запущен, повторный клик теперь ждёт тот же процесс вместо показа нулевого отчёта.',
          'Успешный отчёт сохраняется на Persistent Disk и переживает Deploy/рестарт.',
          'Сначала аудит восстанавливает точные семейства по group_id из offer поставщика, если он есть в XML.',
          'Семьи «review» не входят в расчёт безопасной экономии и требуют ручной проверки перед объединением.'
        ]
      });
      saveVariantAuditSnapshot();
    }catch(e){
      // Do not destroy a previously successful snapshot with a transient empty/API failure.
      variantState.error=safeText(e);
      variantState.supplier_audit_error=supplierAuditError||variantState.supplier_audit_error||null;
    }finally{
      variantState.running=false;
      variantState.finished_at=new Date().toISOString();
    }
    return variantState;
  })();
  try{ return await variantAuditPromise; }
  finally{ variantAuditPromise=null; }
}

function renderVariantReport(){
  const v=variantState;
  const fams=(v.deep_families||v.families||[]);
  const rows=fams.slice(0,300).map((f,i)=>{
    const badge=f.confidence==='high'?'✅ высокая':(f.confidence==='medium'?'🟡 средняя':'👀 review');
    const dims=[f.sizes?.length?`размеры: ${f.sizes.join(', ')}`:'',f.colors?.length?`цвета: ${f.colors.join(', ')}`:''].filter(Boolean).join(' · ');
    const names=(f.names||[]).map(n=>`<div>${autoEscHtml(n)}</div>`).join('');
...
function variantProbeFamilyCoherent(items,vg){
  ...
  // Every Prom item must also resemble ITS matched XML offer, not merely share a numeric id.
  // This catches collisions such as Call Dragon jackets being matched to LOWA shoes.
  for(let i=0;i<items.length;i++){
    const ev=supplierIdentityEvidence(items[i],matches[i]?.record||{});
    const reason=String(matches[i]?.reason||'').toLowerCase();
    if(ev.typeConflict) return {ok:false,reason:'model_conflict',detail:'type_conflict',prom:items[i]?.name,xml:matches[i]?.record?.name};
    if(!reason.includes('точный sku/артикул') && !ev.titleCorroborates){
      return {ok:false,reason:'model_conflict',detail:'prom_xml_title_conflict',prom:items[i]?.name,xml:matches[i]?.record?.name};
    }
  }
...
async function selectVariantProbeFamily(){
  ...
  for(let attempt=0;attempt<4;attempt++){
    try{ products=await listAllProducts(); if(products.length) break; }
    catch(e){ lastProductsError=e; }
    if(attempt<3) await new Promise(r=>setTimeout(r,1200*(attempt+1)));
  }
  if(!products.length) throw new Error('Не удалось получить товары Prom для теста после 4 попыток: '+safeText(lastProductsError));
  promRawCache=new Map(products.map(p=>[String(p.id),p]));

  const supplierDiag=await robustSyncSuppliersForVariants(products,{tries:5});
  const fresh=auditVariantFamilies(products);
  const exactFresh=fresh.families.filter(f=>f.confidence==='high' && f.source==='supplier_group_id' && f.count>=2);

  // Refresh the visible report from the same exact data the probe is using.
  const counted=fresh.families.filter(x=>x.confidence==='high' || x.confidence==='medium');
  const allHigh=fresh.families.filter(x=>x.confidence==='high');
  const allMedium=fresh.families.filter(x=>x.confidence==='medium');
  Object.assign(variantState,{
    running:false,error:null,supplier_audit_error:null,total_positions:products.length,
    standalone_positions:fresh.standalone.length,existing_variant_positions:fresh.existing.length,
    existing_variant_groups:fresh.existingGroups.size,candidate_families:fresh.families.length,
    high_confidence_families:allHigh.length,medium_confidence_families:allMedium.length,
    deep_candidate_families:counted.length,deep_high_confidence_families:allHigh.length,
    deep_medium_confidence_families:allMedium.length,
    deep_supplier_groupid_families:exactFresh.length,
    deep_supplier_groupid_positions:exactFresh.reduce((n,x)=>n+x.count,0),
    deep_supplier_groupid_savings:exactFresh.reduce((n,x)=>n+x.savings,0),
    supplier_records_with_groupid:Object.values(supplierRecords||{}).flat().filter(r=>numericVariationGroupId(r?.variation_group_id||'')).length,
    supplier_matches_for_audit:supplierMatches.size,
    estimated_savings_deep:counted.reduce((n,x)=>n+x.savings,0),
    estimated_cards_deep:Math.max(0,products.length-counted.reduce((n,x)=>n+x.savings,0)),
    goal_gap_after_deep:Math.max(0,Math.max(0,products.length-counted.reduce((n,x)=>n+x.savings,0))-variantState.target_limit),
    need_to_reduce:Math.max(0,products.length-variantState.target_limit),deep_families:fresh.families,
    families:fresh.families.filter(x=>x.source==='strict'),finished_at:new Date().toISOString()
  });
  saveVariantAuditSnapshot();
  ...
  stats.too_large=Math.max(0,allExact.length-candidates.length);

  for(const f of candidates){
    const vg=numericVariationGroupId(f.supplier_variation_group_id);
    if(!vg){ stats.bad_vg++; continue; }
    ...
    const currentGids=uniq(items.map(p=>String(autoGroupId(p)||'')).filter(Boolean));
    const currentNames=uniq(items.map(p=>productGroupName(p,maps)).filter(Boolean));
    let targetGroupId='';
    let targetGroupName='';
    ...
    const sel={family:f,items,variation_group_id:vg,current_group_ids:currentGids,current_group_names:currentNames,target_group_id:String(targetGroupId),target_group_name:targetGroupName||f.group||'PrimeTac',current_group_id:currentGids.length===1?currentGids[0]:'',current_group_name:currentNames.length===1?currentNames[0]:'несколько групп',move_group:moveGroup,dimensions:dims.dims};
    stats.selected={base:f.base,count:f.count,move_group:moveGroup,target_group_id:String(targetGroupId),current_group_ids:currentGids};
...
function buildVariantProbeYml(sel){
  const catId=String(sel.target_group_id||sel.current_group_id||'1');
  const catName=norm(sel.target_group_name||sel.current_group_name||'PrimeTac');
...
async function renderVariantProbePreview(){
  const sel=await selectVariantProbeFamily();
  const f=sel.family;
  const rows=sel.items.map((p,i)=>{
    const m=variantSupplierMatchOf(p)||{};
    const ev=supplierIdentityEvidence(p,m.record||{});
    const sim=Math.round((ev.jac||0)*100);
    return `<tr><td>${i+1}</td><td>${autoEscHtml(p.name||'')}</td><td>${autoEscHtml(m.record?.name||'—')}<div class="m">${autoEscHtml(m.reason||'—')} · схожесть ${sim}%</div></td><td>${autoEscHtml(autoExternalId(p)||'—')}</td><td>${autoEscHtml(sel.dimensions.map(d=>d.name+': '+d.values[i]).join(' · '))}</td></tr>`;
  }).join('');
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PrimeTac — preview variant probe</title><style>:root{color-scheme:dark}body{font-family:system-ui;background:#08100b;color:#eef4ef;padding:12px}a{color:#9de38d}.c{background:#141d17;border:1px solid #304238;border-radius:14px;padding:12px;margin:10px 0}.m{font-size:12px;color:#9caf9f;line-height:1.45}.warn{color:#e8c66c}button{padding:14px 18px;border:0;border-radius:12px;background:#35653a;color:#fff;font-weight:800}table{width:100%;border-collapse:collapse;font-size:12px}th,td{border-bottom:1px solid #304238;padding:8px;text-align:left;vertical-align:top}</style></head><body><h2>🧪 Подготовлен безопасный тест одной семьи</h2><p><a href="/variants/report">← К аудиту</a></p><div class="c"><b>${autoEscHtml(f.base)}</b><div class="m">Источник: точный group_id из XML поставщика · group_id ${autoEscHtml(sel.variation_group_id)} · ${f.count} позиций · экономия после объединения ${f.savings}</div><div class="m">Текущие группы Prom: ${autoEscHtml((sel.current_group_ids||[]).join(', ')||'—')}. Целевая группа: ${autoEscHtml(sel.target_group_name)} (${autoEscHtml(sel.target_group_id)}). ${sel.move_group?'Для этой тестовой семьи потребуется привести позиции к одной уже существующей группе.':'Все позиции уже находятся в одной целевой группе.'}</div></div><div class="c warn"><b>Изменение реальное, но ограничено одной семьёй.</b><div class="m">Будет отправлен YML только для этих ${f.count} существующих товаров. В настройках импорта only_update=true. Цена, наличие, остатки, фото и описания не меняются. Если позиции семьи сейчас разбросаны по группам, разрешено изменить только их группу каталога на показанную выше целевую группу. Общий YML group_id нужен для объединения в разновидности. Если Prom не покажет связь после теста, массовый режим останется заблокирован.</div></div><div class="c"><table><tr><th>#</th><th>Товар Prom</th><th>Товар XML</th><th>external_id</th><th>Различие</th></tr>${rows}</table></div><form method="post" action="/variants/probe/run"><button type="submit">🚀 ЗАПУСТИТЬ ТЕСТ ЭТОЙ СЕМЬИ</button></form></body></html>`;
}
...
<title>PrimeTac AUTO v3.4.21 STRICT IDENTITY</title><style>
...
<div class="m">Реальные ID Prom: <b class="${autoState.real_groups_mapped===MANAGED_GROUPS.length?'ok':'warn'}">${autoState.real_groups_mapped||0}/${MANAGED_GROUPS.length}</b> · не найдено ${autoState.real_groups_missing||0} · неоднозначно ${autoState.real_groups_ambiguous||0}.</div>
<div class="m">Тест переноса 1 товара: ${autoState.group_probe?.ok?'✅ подтверждён':(autoState.group_probe?.status?'⚠️ '+autoEscHtml(autoState.group_probe.status):'не запускался')}${autoState.group_probe?.product_name?' · '+autoEscHtml(autoState.group_probe.product_name):''}.</div>
...
<div class="m" style="margin-top:7px">v3.4.18 сам пересобирает XML-семьи перед тестом и сохраняет глубокий read-only аудит, а запись разрешена только отдельной кнопкой теста одной точной XML <code>group_id</code>-семьи. Массового восстановления в этой версии нет.</div>
...
<div class="m">Фактически совпадает с планом групп: ${autoState.managed_groups_verified||0}/${autoState.managed_groups_total||total||0}. Узлов PrimeTac найдено в Prom: ${autoState.managed_group_nodes_found||0}/${MANAGED_GROUPS.length}. Активных узлов по текущему распределению: ${autoState.managed_group_nodes_expected_found||0}/${autoState.managed_group_nodes_expected||0}. ${autoState.import_background?'Prom продолжает импорт в фоне; новый импорт заблокирован.':''}</div>
<div class="m">Групп Prom прочитано: ${autoState.groups_audited||0}; страниц /groups/list: ${autoState.groups_pages_audited||0}. v3.4.11 считает отдельно всю фиксированную структуру и только реально нужные сейчас узлы; импорт отправляется только по товарам, которые уже есть в текущем каталоге Prom.</div>
...
<div class="m">Status API: пустые <code>errors: []</code> не считаются ошибкой. ID импорта распознаётся из <code>processed_id</code>. Проверка групп учитывает, что <code>categoryId</code> в YML — идентификатор внутри файла импорта, а Prom может выдать группе другой внутренний ID. v3.4.11 больше не пытается создавать уже существующие группы через YML. Сначала имена групп сопоставляются с реальными ID из <code>/groups/list</code>, затем отдельный безопасный тест одного товара проверяет изменение <code>group_id</code> через <code>/products/edit</code>. Массовый перенос до успешного теста заблокирован.</div>
...
<title>PrimeTac AUTO v3.4.21 STRICT IDENTITY</title>
<style>
...
<body><div class="w">
<h2>🧰 PrimeTac Card Manager <span class="m">v3.4.21 STRICT IDENTITY</span></h2>
...
app.get('/variants/audit', async (_req,res)=>{
  await runVariantAudit();
  res.redirect(303,'/variants/report');
});
...
app.get('/groups/refresh', async (_req,res)=>{
  ...
  catch(e){ autoState.errors.unshift({where:'real-groups-refresh',error:safeText(e)}); }
  res.redirect(303,'/');
});

app.get('/groups/probe', async (_req,res)=>{
  await runRealGroupProbe();
  res.redirect(303,'/');
});
app.post('/groups/probe', async (_req,res)=>{
  await runRealGroupProbe();
  res.redirect(303,'/');
});

app.get('/groups/real-map.json', async (_req,res)=>{
  try{
    const r=await refreshRealPromGroupMapping();
    res.json({mapped:autoState.real_groups_mapped,missing:r.missing,ambiguous:r.ambiguous,mapping:r.mapping,groups_read:r.groups.length,probe:autoState.group_probe});
  }catch(e){ res.status(500).json({error:safeText(e)}); }
});

app.get('/groups/real-map', async (_req,res)=>{
  try{
    const r=await refreshRealPromGroupMapping();
    const rows=MANAGED_GROUPS.map(g=>{
      const m=r.mapping[String(g.id)];
      const status=m?.prom_id?'✅':(m?.status==='ambiguous'?'⚠️':'❌');
      return `<tr><td>${status}</td><td>${autoEscHtml(g.name)}</td><td>${autoEscHtml(m?.prom_name||'—')}</td><td>${autoEscHtml(m?.prom_id||'—')}</td><td>${autoEscHtml(m?.parent_prom_id||'—')}</td></tr>`;
    }).join('');
    res.type('html').send(`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PrimeTac real Prom groups</title><style>body{font-family:system-ui;background:#0b100d;color:#eef4ef;padding:12px}table{width:100%;border-collapse:collapse;font-size:12px}td,th{border-bottom:1px solid #2b352f;padding:8px;text-align:left}a{color:#8fd37c}</style></head><body><h2>Реальные группы Prom</h2><p>Сопоставлено ${autoState.real_groups_mapped}/${MANAGED_GROUPS.length}; не найдено ${autoState.real_groups_missing}; неоднозначно ${autoState.real_groups_ambiguous}. Прочитано групп Prom: ${r.groups.length}.</p><p><a href="/groups/probe">🧪 Тест переноса 1 товара</a> · <a href="/">Назад</a></p><table><tr><th></th><th>PrimeTac</th><th>Prom</th><th>Prom ID</th><th>Parent ID</th></tr>${rows}</table></body></html>`);
  }catch(e){ res.status(500).type('text').send(safeText(e)); }
});
...
app.get('/health', (_req,res) => {
  res.json({
    ok: true,
    app: 'PrimeTac AUTO v3.4.21 STRICT IDENTITY',
...
app.listen(PORT, () => {
  console.log(`PrimeTac AUTO v3.4.21 STRICT IDENTITY started on ${PORT}`);
  autoState.next_run_at=new Date(Date.now()+AUTO_INTERVAL_HOURS*3600*1000).toISOString();
  stockState.next_run_at=new Date(Date.now()+STOCK_INTERVAL_HOURS*3600*1000).toISOString();
