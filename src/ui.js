...
function renderDashboard({ state, config }) {
  ...
  const body = r ? `
    <div class="grid">
      ${card("Товарів Prom", r.prom.products)}
      ${card("Груп Prom", r.prom.groups)}
      ${card("BEZET", r.suppliers.BEZET.offers, r.suppliers.BEZET.ok ? "OK" : "ПОМИЛКА")}
      ${card("Militaris", r.suppliers.MILITARIS.offers, r.suppliers.MILITARIS.ok ? "OK" : "ПОМИЛКА")}
      ${card("Точних збігів", r.matching.exactMatches)}
      ${card("Не зіставлено", r.matching.unmatchedPromProducts)}
      ${card("Сімей різновидів XML", r.variants.totalFamilies)}
      ${card("Товарів у сім'ях", r.variants.totalVariantOffers)}
    </div>

    <section>
      <h2>Різновиди</h2>
      <div class="row">
        <span>BEZET: <b>${esc(r.variants.bySupplier.BEZET)}</b></span>
        <span>Militaris: <b>${esc(r.variants.bySupplier.MILITARIS)}</b></span>
        <span>Сімей з точним Prom↔XML збігом: <b>${esc(r.variants.matchedFamilies)}</b></span>
      </div>
    </section>
    ...
    <section>
      ...
      <div class="tablewrap">
        <table>
          ...
          <tbody>
          ${r.samples.families.slice(0,15).map(f => `
            <tr>
              <td>${esc(f.supplier)}</td>
              <td>${esc(f.groupId)}</td>
              <td>${esc(f.count)}</td>
              <td>${esc(f.sample?.[0]?.name || "")}</td>
            </tr>`).join("") || `<tr><td colspan="4">Немає даних</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
  ...
  const error = state.lastError ? `<section class="error"><h2>Остання помилка</h2><pre>${esc(state.lastError)}</pre></section>` : "";
...
<title>PrimeTac Sync v1.0</title>
<style>
